from pathlib import Path
from collections import deque
import csv
import numpy as np
import torch

from env.f1_env import F1StrategyEnv
from agents.dqn.dqn_agent import DQNAgent, DQNConfig
from agents.dqn.action_mask import get_action_mask


CSV_COLUMNS = [
    "episode",
    "episode_return",
    "mean_return_50",
    "final_position",
    "mean_position_50",
    "pit_count",
    "mean_pits_50",
    "mean_loss",
    "epsilon",
    "replay_size",
    "total_steps",
    "track",
    "year",
    "race_name",
]


def set_global_seed(seed: int) -> None:
    np.random.seed(seed)
    torch.manual_seed(seed)

    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)


def train_dqn(
    total_episodes: int = 1500,
    seed: int = 42,
    checkpoint_dir: str = "checkpoints_1/dqn/checkpoints_v1",
) -> None:
    set_global_seed(seed)

    env = F1StrategyEnv()

    config = DQNConfig(
        obs_dim=15,
        action_dim=6,
        gamma=0.99,
        lr=1e-4,
        batch_size=128,
        replay_capacity=100_000,
        learning_starts=2_000,
        train_every=1,
        tau=0.005,
        gradient_clip=10.0,
        epsilon_start=1.0,
        epsilon_final=0.05,
        epsilon_decay_steps=80_000,
        reward_scale=0.01,
        per_alpha=0.6,
        per_beta_start=0.4,
        per_beta_frames=100_000,
        seed=seed,
    )

    agent = DQNAgent(config)

    checkpoint_path = Path(checkpoint_dir)
    checkpoint_path.mkdir(parents=True, exist_ok=True)

    csv_path = checkpoint_path / "training_metrics.csv"

    recent_returns = deque(maxlen=50)
    recent_positions = deque(maxlen=50)
    recent_pits = deque(maxlen=50)

    best_mean_return = -float("inf")

    with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=CSV_COLUMNS)
        writer.writeheader()

        for episode in range(1, total_episodes + 1):
            obs, info = env.reset(seed=seed + episode)

            done = False
            episode_return = 0.0
            episode_loss_values = []
            pit_count = 0

            while not done:
                action_mask = get_action_mask(env)
                action = agent.select_action(
                    obs,
                    evaluation=False,
                    action_mask=action_mask,
                )

                next_obs, reward, terminated, truncated, info = env.step(action)
                done = terminated or truncated
                next_action_mask = get_action_mask(env)

                # This follows the action definition already used in your files:
                # action 0 = no pit, every other action = a pit action.
                if action != 0:
                    pit_count += 1

                agent.store(
                    obs=obs,
                    action=action,
                    reward=reward,
                    next_obs=next_obs,
                    done=done,
                    next_action_mask=next_action_mask,
                )

                agent.total_steps += 1

                metrics = agent.train_step()
                if metrics is not None:
                    episode_loss_values.append(float(metrics["loss"]))

                obs = next_obs
                episode_return += float(reward)

            final_position = int(env.state.end_position)

            recent_returns.append(episode_return)
            recent_positions.append(final_position)
            recent_pits.append(pit_count)

            mean_return_50 = float(np.mean(recent_returns))
            mean_position_50 = float(np.mean(recent_positions))
            mean_pits_50 = float(np.mean(recent_pits))

            # NaN means that learning had not started during this episode.
            # This is more accurate than plotting a false loss value of zero.
            mean_loss = (
                float(np.mean(episode_loss_values))
                if episode_loss_values
                else float("nan")
            )

            epsilon = float(agent.epsilon())
            replay_size = int(len(agent.replay))
            total_steps = int(agent.total_steps)

            writer.writerow(
                {
                    "episode": episode,
                    "episode_return": episode_return,
                    "mean_return_50": mean_return_50,
                    "final_position": final_position,
                    "mean_position_50": mean_position_50,
                    "pit_count": pit_count,
                    "mean_pits_50": mean_pits_50,
                    "mean_loss": mean_loss,
                    "epsilon": epsilon,
                    "replay_size": replay_size,
                    "total_steps": total_steps,
                    "track": env.track,
                    "year": env.year,
                    "race_name": env.name,
                }
            )

            # Protect logged results if training is interrupted unexpectedly.
            csv_file.flush()

            loss_text = f"{mean_loss:.5f}" if np.isfinite(mean_loss) else "N/A"

            print(
                f"Episode {episode:04d} | "
                f"Return {episode_return:9.2f} | "
                f"Mean50 Return {mean_return_50:9.2f} | "
                f"Final P{final_position:02d} | "
                f"Mean50 Pos {mean_position_50:5.2f} | "
                f"Pits {pit_count:02d} | "
                f"Mean50 Pits {mean_pits_50:5.2f} | "
                f"Loss {loss_text} | "
                f"Epsilon {epsilon:.3f} | "
                f"Replay {replay_size} | "
                f"{env.track} {env.year} {env.name}"
            )

            if episode % 50 == 0:
                latest_path = checkpoint_path / "latest.pt"
                agent.save(str(latest_path))
                print(f"Saved latest checkpoint: {latest_path}")

            if len(recent_returns) == recent_returns.maxlen:
                if mean_return_50 > best_mean_return:
                    best_mean_return = mean_return_50
                    best_path = checkpoint_path / "best.pt"
                    agent.save(str(best_path))
                    print(
                        f"Saved best checkpoint: {best_path} | "
                        f"Mean50 Return: {best_mean_return:.2f}"
                    )

    final_path = checkpoint_path / "final.pt"
    agent.save(str(final_path))

    print(f"Training complete. Final model saved to: {final_path}")
    print(f"Training metrics saved to: {csv_path}")


if __name__ == "__main__":
    train_dqn(
        total_episodes=1500,
        seed=42,
        checkpoint_dir="checkpoints_1/dqn/checkpoints_v1",
    )
