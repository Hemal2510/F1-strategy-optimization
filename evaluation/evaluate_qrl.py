...
def evaluate_checkpoint(
    ...
    agent = QRLAgent(QRLConfig(obs_dim=15, action_dim=6, n_qubits=8, seed=seed))
 
    agent.load(checkpoint_path)
    agent.online_net.eval()#this sets the network to evaluation mode, which disables dropout and batch normalization layers, ensuring consistent behavior during evaluation.
