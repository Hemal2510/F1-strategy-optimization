...
if "action" in race_df.columns and "action_name" in race_df.columns:
    action_names_map = race_df[["action", "action_name"]].drop_duplicates().set_index("action")["action_name"].to_dict()
else:
    action_names_map = {}

cols = st.columns(max(len(agents_in_race), 1))
for col, agent in zip(cols, agents_in_race):
    with col:
        ...
        action_name = row.get("action_name", f"action {row.get('action', '?')}")
        st.metric("Action taken", action_name)
        ...
        if n_actions and pd.notna(row.get("q_0", np.nan)):
            ...
            mask_vals = [bool(row.get(f"mask_{i}", True)) for i in range(n_actions)]
