...
@dataclass
class QRLConfig:
#Config of QRL agent - includes all the hyperparameters
    obs_dim: int = 15
    action_dim: int = 6
    n_qubits: int = 8
 
    gamma: float = 0.99
    lr: float = 5e-3#lr increased as compared to DQN agent to speed up the training of the quantum circuit
 
    batch_size: int = 32#batch size reduced as compared to DQN agent to reduce the memory requirements of the quantum circuit
...
class QRLAgent:
    ...
    Double DQN + Dueling head + Prioritized Replay, with a hybrid quantum-classical network in place of the classical DQNNetwork.

    This class is responsible for:
        1. Creating the online and target networks
        2. Selecting actions
        3. Storing experience
        4. Training the QRL agent
        5. Updating replay priorities
    ...
    #function to compute the epsilon value for epsilon-greedy action selection
    ...
    #function to select an action based on the current observation, using epsilon-greedy strategy
    @torch.no_grad()
    def select_action(
        self,
        obs: np.ndarray,
